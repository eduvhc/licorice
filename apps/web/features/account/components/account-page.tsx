...
async function AccountPage({ error, session }: AccountPageProps) {
    ...
    <main className="mx-auto flex min-h-svh w-full max-w-3xl flex-col gap-8 px-6 py-12 lg:px-10">
      ...
      <Card>
        ...
        <CardContent className="grid gap-4 sm:grid-cols-2">
          ...
          <div className="rounded-xl border p-4">
            <p className="text-sm text-muted-foreground">{t("sessionExpires")}</p>
            <p className="mt-1 font-medium">{formatDate(session.session.expiresAt)}</p>
          </div>
          <div className="rounded-xl border p-4">
            <p className="text-sm text-muted-foreground">{t("signedInAt")}</p>
            <p className="mt-1 font-medium">{formatDate(session.session.createdAt)}</p>
          </div>
          <div className="rounded-xl border p-4">
